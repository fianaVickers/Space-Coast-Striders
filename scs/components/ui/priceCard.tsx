...
export const PriceCard: React.FC<{
  ...
  return (
    <article
      className={cn(
        'text-start border backdrop-blur-[4px] h-full min-h-[720px] w-full max-w-[350px]  dark:border-neutral-800 border-neutral-300 px-10 py-10 rounded-xl relative z-[1] flex flex-col justify-between',
        className
      )}
    >
      <span>{children}</span>
      <Link
        ...
        className={
          'w-full flex items-center bg-orange-500 justify-center mt-7 p-1.5 rounded-lg border dark:border-neutral-700 border-neutral-300 dark:text-neutral-300 text-white'
        }
      >
        Register
      </Link>
...
export const ProductName: React.FC<{ children: React.ReactNode; className?: string }> = ({
  children,
  className
}) => <h1 className={cn('text-lg text-gray-500 font-bold  text-start', className)}>{children}</h1>

export const Price: React.FC<{ children: React.ReactNode; className?: string }> = ({
  children,
  className
}) => (
  <div className={cn('mt-10 mb-5', className)}>
...
export const Description: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className={'h-20'}>
    <p className={'leading-[2] text-lg'}>{children}</p>
  </div>
)
